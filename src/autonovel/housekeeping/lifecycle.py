...
def _next_step_for(series: SeriesLayout, book: str) -> object:
    ...
    state = PipelineState(
        ...
        # If the book entry pre-declares a status more advanced than the
        # filesystem suggests (e.g. the user manually marked it), respect
...
def _infer_phase(series: SeriesLayout, book_root: Path) -> tuple[str, int]:
    ...
    if _is_populated(voice, min_chars=400, exclude_markers=("(empty", "Generated by")):
        return ("foundation", 0)
    ...
    for fname in ("world.md", "characters.md", "canon.md"):
        if _is_populated(series.shared / fname, min_chars=120,
                         exclude_markers=("(empty", "Generated by")):
            return ("foundation", 0)
