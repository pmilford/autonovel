...
- `.autonovel/in-progress.lock` (PID-bound, useless on another
  machine)
- `books/<book>/typeset/*.pdf`, `*.epub`, `novel.tex`,
  `chapters_combined.md`, `front_matter.tex`,
  `chapters_content.tex`, `metadata.yaml` (regenerated by
  `/autonovel:typeset`; opening one stale PDF in your repo
  history wastes nothing because rebuild is one command)
