...
| `/autonovel:impact-of [--book <short-name>] [--source promote-canon] [--format markdown\|json]` | light | After a foundation mutation (today: `/autonovel:promote-canon` only), parse `## Superseded` blocks in `shared/canon.md`, extract tokens unique to each prior value, grep every chapter for them, and emit a per-chapter action checklist of `/autonovel:revise --chapter N` calls with line-snippet evidence. Kills the `ls` + `grep` "what should I revise now?" workflow. Mechanical candidate generator — review list with snippets, not a quality gate (the LLM-side semantic scan is queued in FUTURE-TODOS). |
