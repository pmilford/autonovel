...
| `/autonovel:teaser-render --book <short-name> [--provider stub\|grok\|kie\|veo\|magichour\|fal\|flow\|pollinations] [--kind auto\|image\|video] [--takes <n>] [--shot <id>] [--height <px>] [--token <key>] [--delay <s>] [--dry-run]` | standard | Render the shot prompts into **actual clips**. Validates the whole chain for **$0/zero-quota** via the offline **`stub`** backend (local placeholder keyframes) before spending a real one; **`grok`** (free dialogue+music, no card — needs `XAI_API_KEY`) is the default real video backend. See [`teaser-render-providers.md`](teaser-render-providers.md). **Stateless** — clips land in `books/<book>/teaser/clips/`, no state file, nothing assembled. `--dry-run` prints the request plan + key status for $0; pacing/backoff is automatic. Then a **vision clip critique** marks each clip KEEP / REGENERATE / UPGRADE-TO-PAID into `clips/render-report.md`; paid providers are only ever *recommended*, never auto-called. |
...
| `teaser-render <teaser.json> [--out-dir <d>] [--provider <p>] [--kind auto\|image\|video] [--takes <n>] [--shot <id>] [--height <px>] [--token <k>] [--delay <s>] [--max-retries <n>] [--dry-run]` | Render clips. `stub` = offline placeholder keyframes (no network/key/quota — validate the pipeline free); `grok` (default) / `kie` / `veo` / `magichour` / `fal` = async create→poll→download video backends; `flow` = manual (GUI, prints import steps); `pollinations` = free `flux` keyframe images only. Key from `--token` → env → `.env`. `--dry-run` builds the plan + key status; pacing + 429/503 backoff automatic; failures isolate per clip, a 402/auth wall stops the batch. No assembly. See [`teaser-render-providers.md`](teaser-render-providers.md). |
