...
| `/autonovel:dialogue [--book <short-name>] [--summary-only] [--format markdown\|json]` | light | Per-chapter dialogue-mechanics linter — adverb-heavy speech tags ("she said quietly"), said-bookisms (`exclaimed`, `murmured`, `whispered`), and stutters (the same non-`said` verb 3+ times within 10 lines). Pure mechanical pre-flight before a heavy-tier evaluate run. |
