...
| `teaser-critique <teaser.json> [--provider <p>]` | Mechanical pre-generation critique (advisory flags). **Story-spine (Phase 6):** no-dramatic-question, no-logline, no-stakes, no-emotional-arc, thin-dialogue (<2 spoken lines on an audio provider), thin-text-cards (<2 cards). **Prompt:** appearance-drift, thin-prompt, no-palette/-reference, multi-action, audio-unsupported, missing hook/button, length-mismatch. |
