...
| `/autonovel:teaser-assemble --book <short-name> [--kind image\|video] [--audio <path>] [--audio-mode auto\|duck\|mix\|clip-only\|bed-only\|none] [--no-clip-audio] [--fps <n>] [--take <n>] [--force]` | standard | Stitch the rendered clips into **one teaser video** via ffmpeg. Builds an editable `cut_list.json` (ordered clips + durations + text-card notes + audio policy), runs ffmpeg (v1: hard cuts; image slideshow or video clips), then a **viewer-panel cut critique** → `assembly-report.md`. **Audio (5.4):** native clip dialogue/music is preserved; a `--audio` music bed **ducks under the dialogue** by default (`--audio-mode` to override). No burned-in text (cards go in an editor); ffmpeg required. |
...
| `teaser-cut-list <teaser.json> [--clips-dir <d>] [--kind image\|video] [--audio <p>] [--audio-mode <m>] [--no-clip-audio] [--fps <n>] [--take <n>]` | Build an editable `cut_list.json` from teaser.json + the clips on disk (ordered clips + durations + text-card notes + audio policy: keep native clip audio, duck/mix/replace a bed). Plan only; reports shots with no clip. |
