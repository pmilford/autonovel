...
| `/autonovel:impact-of [--book <short-name>] [--source promote-canon\|research] [--with-llm] [--format markdown\|json]` | light | After a foundation mutation, list the chapters that reference the now-wrong fact and emit an action checklist of `/autonovel:revise --chapter N` calls with line-snippet evidence. Kills the `ls` + `grep` "what should I revise now?" workflow. Default mode is mechanical (parse `## Superseded` blocks in `shared/canon.md`, grep tokens unique to each prior value across chapters). With `--with-llm`, a Haiku-tier classifier labels each match HIGH (real direct fix) / MEDIUM (implied — careful rewrite) / LOW (token coincidence) / FALSE_POSITIVE so the action checklist only includes HIGH+MEDIUM. With `--source research`, reads notes newer than the last canon timestamp and per-chapter scans for contradictions against their `## Candidate Canon Entries`. Read-only by contract. |
...
| `autonovel doctor [--fix]` | Sanity-check the series — required dirs, project.yaml shape, missing external CLI tools (warnings only). |
...
| `autonovel tui [--book <name>]` | Long-running read-only terminal UI — chapters / scores / research / front matter / reviews / next actions / live help. Tabbed; auto-refreshes every 5 s. Never acquires the lock — safe to run alongside an active sweep. Requires `pip install 'autonovel[tui]'` (or `pipx inject autonovel textual`). Run in a *separate* terminal from your runtime. |
...
| `autonovel install [--no-model-pin]` | Write `/autonovel:*` command files into every detected runtime's expected path (idempotent). `--no-model-pin` omits per-command model frontmatter so the runtime's session model wins. |
