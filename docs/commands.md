...
| `/autonovel:teaser --book <short-name> [--length 30\|60\|90\|120\|180] [--provider <name>] [--with-treatment] [--fresh] [--revise-rounds <n>] [--no-revise] [--force]` | standard | **The one-command teaser pipeline.** `--fresh` starts over — archives every teaser artifact EXCEPT the approved `refs/` + `refs.yaml` (to `teaser/reset-archive/`) then rebuilds (implies `--force`). Chains `teaser-beats` → `shot-prompts` → **the `teaser-critique` ⟳ `teaser-revise` loop** (applies the critique in place until the **render gate is READY** or `--revise-rounds` is spent; `--no-revise` skips it), each in a fresh `task` subagent (context hygiene), and prints one combined summary that includes the render-gate verdict. `--with-treatment` runs `/autonovel:treatment` first when none exists. Produces `beats.md` + `teaser.json` + per-shot files + `critique.md`. Free; stops before any render. |
| `/autonovel:teaser-beats --book <short-name> [--length 30\|60\|90\|120\|180] [--provider generic\|veo\|sora\|runway\|kling\|luma\|pollinations] [--force]` | standard | Fix the **story spine** (dramatic question, logline, want vs. opposing force, emotional arc, score direction), then select 8–20 teaser beats on the hook → escalation → title → button arc — each serving the question and raising the stakes — from the treatment/outline + eval_logs, to a budget from `teaser-plan`. Writes the hand-editable `books/<book>/teaser/beats.md` (with a `## Spine` block). Free; no generation. `--force` archives the prior `beats.md` to `teaser/script-takes/` first. |
| `/autonovel:shot-prompts --book <short-name> [--provider <name>] [--length <s>] [--force]` | heavy | Turn the spine + beat-sheet into provider-ready, heavily-described shot prompts. Copies the `spine` into `teaser.json`, **mines 3–6 loaded dialogue lines** from the manuscript (bp 5), authors 2–4 **text cards** carrying the premise (bp 6), and fills the structured shot schema (verbatim appearance, palette lock, one action/one move, content-word negative prompt, consistency anchors). Runs a **free pre-generation critique** (mechanical `teaser-critique` + an LLM rewrite pass), then writes `books/<book>/teaser/teaser.json` + per-shot `books/<book>/teaser/shots/shot_<id>.md`. No generation cost. `--force` archives the prior `teaser.json` to `teaser/script-takes/` first; `teaser/refs/` originals are reused untouched. |
| `/autonovel:teaser-revise --book <short-name> [--provider <name>] [--max-rounds <n>]` | heavy | **Applies the critique's findings to `teaser.json` IN PLACE** — the "revise" half of the teaser loop (mirrors how the book pipeline acts on `evaluate`). Reads `teaser/critique.md` + the mechanical flags and fixes only what's flagged (fills the spine, strengthens dialogue/cards, repairs 4-act order / stakes ladder, rewrites weak shots), **preserving everything else**; archives the prior script, re-validates, and re-critiques up to `--max-rounds`. Distinct from `shot-prompts --force` (a blind regenerate-from-scratch). No generation cost. |
| `/autonovel:teaser-critique --book <short-name> [--provider <name>]` | standard | Standalone, re-runnable **free pre-generation critique** of a (hand-edited) `teaser.json`: the mechanical linter + an LLM critic pass that judges the **story spine first** (does it pose a question and withhold the answer? do beats escalate? does the dialogue/cards make the story legible?), then each shot against trailer craft and the beat it serves. **Read-only** on `teaser.json`; writes an advisory `books/<book>/teaser/critique.md` (with a `## Story spine` section) and concrete rewrite suggestions. |
...
| `teaser-critique <teaser.json> [--provider <p>]` | Mechanical pre-generation critique (advisory flags). **Story-spine = render gate (Phase 6, bp 12):** no-dramatic-question, no-logline, no-stakes, no-emotional-arc, no-genre, thin-dialogue (<2 spoken lines on an audio provider), thin-text-cards (<2 cards). **4-act order:** hook-not-first, multiple-hooks, no-title, button-not-last, title-after-button. **Stakes ladder:** no-stakes-ladder, stakes-not-rising. **Cast:** cast-sprawl (>3 named faces). **Prompt:** appearance-drift, thin-prompt, no-palette/-reference, multi-action, audio-unsupported, length-mismatch. |
