...
Build spec: [`prd-movie-teaser-mode.md`](prd-movie-teaser-mode.md). Creative guide: [`teaser-craft.md`](teaser-craft.md). Backend map + free-tier/key setup: [`teaser-render-providers.md`](teaser-render-providers.md). Full pipeline shipped: `treatment` → `teaser-brief` (distil the through-line + turn + killer lines) → `teaser` (orchestrator: `teaser-beats` → `shot-prompts`) → `teaser-critique` (scores the **interestingness rubric** → `quality.json`) → `teaser-revise` (apply the critique in place, lift the weak quality dimensions + de-boring pass; loop until **both render gates** are READY) → `teaser-refs` (develop + approve character references) → `teaser-render` (offline `stub` to validate free, then `grok` real clips + vision critique) → `teaser-assemble` (ffmpeg stitch + viewer-panel cut critique). The planning commands (treatment → shot-prompts, critique) are **free, no generation**; `teaser-render` validates the whole chain for $0 via the offline `stub` backend before spending a real one; `teaser-assemble` runs ffmpeg locally. **Two render gates (Phase 11):** the structural story gate AND the quality gate (overall ≥ 7, no dimension < 5) — a structurally-complete-but-boring teaser is refused.
...
| `/autonovel:teaser --book <short-name> [--length 30\|60\|90\|120\|180] [--provider <name>] [--with-treatment] [--fresh] [--revise-rounds <n>] [--no-revise] [--force]` | standard | **The one-command teaser pipeline.** `--fresh` starts over — archives every teaser artifact EXCEPT the approved `refs/` + `refs.yaml` (to `teaser/reset-archive/`) then rebuilds (implies `--force`). Runs `teaser-brief` (distil, when a treatment exists) → `teaser-beats` → `shot-prompts` → **the `teaser-critique` ⟳ `teaser-revise` loop** (applies the critique in place AND lifts the weak quality dimensions + de-borings the flattest beats until **both render gates are READY** — story complete AND quality ≥ 7 — or `--revise-rounds` is spent; `--no-revise` skips it), each in a fresh `task` subagent (context hygiene), and prints one combined summary that includes the combined render-gate verdict (story + quality/10). `--with-treatment` runs `/autonovel:treatment` first when none exists. Produces `brief.md` + `beats.md` + `teaser.json` + per-shot files + `critique.md` + `quality.json`. Free; stops before any render. |
| `/autonovel:teaser-beats --book <short-name> [--length 30\|60\|90\|120\|180] [--provider generic\|veo\|sora\|runway\|kling\|luma\|pollinations] [--force]` | standard | Fix the **story spine** (dramatic question, logline, want vs. opposing force, **the midpoint turn/reversal**, emotional arc, score direction, genre), then select 8–20 teaser beats on the hook → escalation → title → button arc — each serving the question and raising the stakes, the escalation **grouped into the `movements`** the plan prints, with ≥1 want + ≥1 cost character beat — from the brief (`teaser/brief.md`)/treatment/outline + eval_logs, to a budget from `teaser-plan`. Writes the hand-editable `books/<book>/teaser/beats.md` (with a `## Spine` block incl. `Turn`). Free; no generation. `--force` archives the prior `beats.md` to `teaser/script-takes/` first. |
| `/autonovel:shot-prompts --book <short-name> [--provider <name>] [--length <s>] [--force]` | heavy | Turn the spine + beat-sheet into provider-ready, heavily-described shot prompts. Copies the `spine` (incl. the `turn`) into `teaser.json`, **mines the length-scaled loaded dialogue lines** from the manuscript (bp 5; ~1 per 20 s, ≈8–10 for 180 s), authors 2–4 **text cards** carrying the premise (bp 6), **stages the turn** in a midpoint escalation shot, tags ≥1 `character_beat: want` + ≥1 `cost` (Phase 11), and fills the structured shot schema (verbatim appearance, palette lock, one action/one move, content-word negative prompt, consistency anchors). Runs a **free pre-generation critique** (mechanical `teaser-critique` + an LLM rewrite pass), then writes `books/<book>/teaser/teaser.json` + per-shot `books/<book>/teaser/shots/shot_<id>.md`. No generation cost. `--force` archives the prior `teaser.json` to `teaser/script-takes/` first; `teaser/refs/` originals are reused untouched. |
| `/autonovel:teaser-revise --book <short-name> [--provider <name>] [--max-rounds <n>] [--deboring]` | heavy | **Applies the critique's findings to `teaser.json` IN PLACE** — the "revise" half of the teaser loop (mirrors how the book pipeline acts on `evaluate`). Reads `teaser/critique.md` + the mechanical flags + the `quality.json` scorecard and fixes only what's flagged (fills the spine incl. the turn, strengthens dialogue/cards, repairs 4-act order / stakes ladder, rewrites weak shots), **plus lifts the weak quality dimensions and runs the de-boring pass** (swap the 3 flattest beats + flattest line for the most dramatic moments/sharpest quotes; forced with `--deboring`), re-scoring `quality.json` — **preserving everything else**; archives the prior script, re-validates, and re-critiques up to `--max-rounds` until both gates pass. Distinct from `shot-prompts --force` (a blind regenerate-from-scratch). No generation cost. |
| `/autonovel:teaser-brief --book <short-name> [--length 30\|60\|90\|120\|180] [--audience xprize\|general] [--force]` | standard | **Distil** the treatment into a one-page teaser brief (Phase 11): the single filmable through-line, the midpoint **turn**, the 3 must-have dramatic moments, the killer lines, the want→cost, and the withheld ending — so `teaser-beats` selects from a sharp brief instead of the whole story. Writes `books/<book>/teaser/brief.md`. Free; no generation; refuses to clobber without `--force`. |
| `/autonovel:teaser-critique --book <short-name> [--provider <name>]` | standard | Standalone, re-runnable **free pre-generation critique** of a (hand-edited) `teaser.json`: the mechanical linter + an LLM critic pass that judges the **story spine first** (does it pose a question and withhold the answer? do beats escalate? does the dialogue/cards make the story legible?), then each shot against trailer craft and the beat it serves, **then scores the eight-dimension interestingness rubric → `books/<book>/teaser/quality.json`** (the Phase-11 quality gate). **Read-only** on `teaser.json`; writes an advisory `books/<book>/teaser/critique.md` (with `## Story spine` + `## Quality score` sections) and concrete rewrite suggestions. The render gate is READY only when the story is complete AND quality clears overall ≥ 7 / no dimension < 5. |
...
| `/autonovel:teaser-render --book <short-name> [--provider stub\|gemini\|grok\|kie\|veo\|magichour\|fal\|flow\|pollinations] [--kind auto\|image\|video] [--refs] [--voices] [--score native\|bed\|none] [--from-keyframes] [--film-style <s>] [--takes <n>] [--shot <id>] [--height <px>] [--token <key>] [--delay <s>] [--dry-run]` | standard | Render the shot prompts into **actual clips/keyframes**. Validates the whole chain for **$0/zero-quota** via the offline **`stub`** backend before spending a real one; **`grok`** (free dialogue+music, no card — `XAI_API_KEY`) is the default real video backend; **`gemini`** generates **reference-conditioned photoreal keyframes** (Nano Banana). `--refs` feeds each shot's **approved** character references (from `teaser-refs`) so identity holds across shots (approval gate: pending subjects skipped); `--from-keyframes` animates each shot from its `shot_<id>.png` keyframe (image-to-video); `--voices` injects each speaker's locked, age-resolved voice descriptor (from `refs.yaml`, picked by the shot's `story_year`) so the voice holds scene-to-scene; `--film-style` swaps the typeset art style for a film look. See [`teaser-render-providers.md`](teaser-render-providers.md). **Stateless** — clips land in `books/<book>/teaser/clips/`. `--dry-run` prints the plan + key status for $0; pacing/backoff automatic. Then a **vision clip critique** marks each KEEP / REGENERATE / UPGRADE-TO-PAID into `clips/render-report.md`; paid providers only ever *recommended*. |
| `/autonovel:teaser-assemble --book <short-name> [--kind image\|video\|mixed] [--audio <path>] [--audio-mode auto\|duck\|mix\|clip-only\|bed-only\|none] [--no-clip-audio] [--no-transitions] [--burn-titles] [--font <path>] [--audio-seam-fade <s>] [--fps <n>] [--take <n>] [--force]` | standard | Stitch the rendered clips into **one teaser video** via ffmpeg. Builds an editable `cut_list.json` (ordered clips + durations + text-card notes + audio policy + transitions), runs ffmpeg, then a **viewer-panel cut critique** → `assembly-report.md`. **Mixed (Phase 8):** `--kind mixed` weaves dynamic `shot_<id>.mp4` shots (native audio) through still `shot_<id>.png` keyframes (silent), normalized to one WxH/AAC. **Audio (5.4):** native clip dialogue/music preserved; a `--audio` bed **ducks under the dialogue** by default. **Transitions (5.7):** open fade-in / close fade-out / title fade by default (`--no-transitions`); `teaser-transitions` suggests where else. **Titles (Phase 8):** `--burn-titles` draws the cards in (drawtext; title centered, stingers lower-third, faded), `--font` picks a serif — else cards stay editor notes. **Versioned (5.8):** the output mp4 is timestamped with a `_latest` pointer. ffmpeg required. |
...
| `teaser-critique <teaser.json> [--provider <p>]` | Mechanical pre-generation critique (advisory flags). **Story-spine = render gate (Phase 6, bp 12):** no-dramatic-question, no-logline, no-stakes, no-emotional-arc, no-genre, thin-dialogue (<2 spoken lines on an audio provider), thin-text-cards (<2 cards). **4-act order:** hook-not-first, multiple-hooks, no-title, button-not-last, title-after-button. **Stakes ladder:** no-stakes-ladder, stakes-not-rising. **Cast:** cast-sprawl (>3 named faces). **Story craft (Phase 11, advisory — the quality gate enforces them):** no-turn (no midpoint reversal in the spine), no-character-arc (no want/cost character beat). **Prompt:** appearance-drift, thin-prompt, no-palette/-reference, multi-action, audio-unsupported, length-mismatch. |
| `teaser-quality <quality.json\|teaser.json> [--template] [--format json\|human]` | Validate the interestingness scorecard + compute the **HARD quality gate (Phase 11)**: overall ≥ 7 AND no dimension < 5 across the eight dimensions (hook_grip, question_sharpness, stakes_escalation, character, dialogue_quality, surprise_turn, coherence, button). Exit 0 = PASS, 3 = BLOCK/missing/un-scored. `--template` prints a blank scorecard scaffold. The *scores* are authored by the LLM judge in `teaser-critique`; this only checks structure + applies the rule in one place (shared with the render gate). |
...
| `teaser-refs-plan <teaser.json> [--refs-dir <d>] [--art-references-dir <d>] [--with-locations]` | Plan the canonical **reference image** per recurring subject (consistency anchor): which shots use each, which already exist (in `teaser/refs/` or a shared `art_references/` plate), which are still missing. `--with-locations` (Phase 7) also plans a plate per distinct `setting`. |
| `teaser-refs <teaser.json> [--manifest <p>] [--art-references-dir <d>] [--init] [--with-locations] [--force]` | **Character/location-reference manifest + approval status** (Phase 5/7). Merges the auto plan with a declared `refs.yaml` (per subject: `source` wikimedia\|local\|generate, locked `appearance`/`constraints`, optional `voice`/`voice_ages` + `appearance_ages` age ladders, `kind` character\|location, `status` pending→approved→locked) and reports the one **next action** each (declare-source / fetch-source / generate / approve / ready) + the approval gate. `--init` scaffolds `refs.yaml`; `--with-locations` (Phase 7) also scaffolds/tracks a period-correct plate per distinct setting. `--init --force` is **non-destructive** — it preserves every declared/locked entry (status, source, appearance, voice/age ladders, ref_path), refreshing only the per-subject shot list and adding new subjects pending. |
...
| `teaser-render <teaser.json> [--out-dir <d>] [--provider <p>] [--kind auto\|image\|video] [--refs] [--voices] [--refs-manifest <p>] [--from-keyframes] [--keyframe-dir <d>] [--score native\|bed\|none] [--film-style <s>] [--takes <n>] [--shot <id>] [--height <px>] [--token <k>] [--delay <s>] [--max-retries <n>] [--no-archive] [--auto-regenerate] [--max-regen <n>] [--skip-narrative-gate] [--dry-run]` | Render clips/keyframes. **Narrative gate (Phase 6, bp 12):** refuses a real render (exit 3) when the teaser has no story spine / thin dialogue/cards — `stub`, single-`--shot`, and `--skip-narrative-gate` are exempt; `--dry-run` reports `narrative_gate_blocks` instead. **Quality gate (Phase 11):** once the story gate is clear, also refuses a real render (exit 3) when `teaser/quality.json` is missing or below the bar (overall ≥ 7, no dimension < 5) — same exemptions; `--dry-run` reports `quality_gate_blocks` + `quality_overall`. **Auto-regenerate:** the vision critique's REGENERATE clips are re-rendered automatically on free `stub`, and on a paid backend only with `--auto-regenerate` (bounded by `--max-regen`, default 3); UPGRADE-TO-PAID is never auto-acted. The critique persists a reviewable `clips/render-report.md` + `render-report.json`; **`--revise`** re-runs against that report, re-rendering only its REGENERATE shots (review-between-runs; the render-side mirror of `teaser-revise`). **Text-free titles:** `role: title` / text-card shots inject a no-legible-type negative so the model renders a clean plate (title burned in at assembly) — the authored `negative_prompt` is now actually sent (folded into the prompt). `stub` = offline placeholder keyframes (no network/key/quota); `gemini` = reference-conditioned photoreal image keyframes (Nano Banana); `grok` (default) / `kie` / `veo` / `magichour` / `fal` = async video backends; `flow` = manual; `pollinations` = free `flux` keyframe images. `--refs` threads each shot's **approved** references (`refs.yaml`) into reference-capable backends (gemini/fal/pollinations-kontext); only approved/locked subjects flow (approval gate). `--from-keyframes` animates each shot from its `shot_<id>.png` keyframe (image-to-video on grok/veo/kie). `--voices` injects each speaker's locked, age-resolved voice descriptor (refs.yaml `voice`/`voice_ages`, picked by `story_year`) into the video prompt. `--score` sets the music policy (native/bed/none — see teaser-render-providers.md). `--film-style` overrides the typeset art style. Key from `--token` → env → `.env`. Every render is archived to `clips/takes/` (versioned takes; `--no-archive` opts out) so an earlier take survives. `--dry-run` builds the plan + key status; pacing + 429/503 backoff automatic; a 402/auth wall stops the batch. No assembly. See [`teaser-render-providers.md`](teaser-render-providers.md). |
| `teaser-cut-list <teaser.json> [--clips-dir <d>] [--kind image\|video\|mixed] [--audio <p>] [--audio-mode <m>] [--no-clip-audio] [--no-transitions] [--burn-titles] [--font <p>] [--audio-seam-fade <s>] [--fps <n>] [--take <n>]` | Build an editable `cut_list.json` from teaser.json + the clips on disk (ordered clips + per-entry `media` for `mixed` + durations + text-card notes + audio policy + transition defaults). `--kind mixed` picks video-else-still per shot; `--burn-titles`/`--font` mark cards for ffmpeg drawtext. Plan only; reports shots with no clip. |
| `teaser-transitions <teaser.json> [--year-gap <n>] [--slow-ratio <r>]` | Suggest where non-cut scene transitions are worth considering — from structured signals (story_year jumps, setting/location changes, fast→slow pace shifts, open/close). Advisory candidate generator; the artistic placement is the LLM's call in `teaser-assemble`. |
| `teaser-takes <teaser.json> [--clips-dir <d>]` | List archived render **takes** per shot (`clips/takes/shot_<id>_take<N>.<ext>`). Re-renders are kept, not overwritten (Phase 5.8). |
| `teaser-take-pick <teaser.json> --shot <id> --take <N> [--clips-dir <d>]` | Promote an archived take back to the latest pointer (`clips/shot_<id>.<ext>`) so the next cut-list/assemble uses it. |
| `teaser-music <teaser.json> [--provider stub\|musicgen\|elevenlabs] [--prompt <p>] [--duration <s>] [--out-dir <d>] [--token <k>] [--dry-run]` | Generate one **cohesive music bed** for the teaser (Phase 9), default prompt = the spine's `score_direction`; writes a versioned `teaser/music/<title>_bed_<UTC>.<ext>` + `_latest`. `stub` = offline silent WAV ($0 chain test); `musicgen` (HF `HF_TOKEN`) / `elevenlabs` (`ELEVENLABS_API_KEY`) generate real audio. Feed to `teaser-assemble --audio`. |
| `teaser-reset <teaser-dir\|teaser.json> [--dry-run]` | **Fresh-run reset:** archive every teaser artifact (beats/teaser.json/shots/clips/critique/cut_list/music/reports) to `teaser/reset-archive/<UTC>/` **except** the approved `refs/` + `refs.yaml`. Non-destructive (moves, never deletes). Used by `/autonovel:teaser --fresh`. |
| `teaser-archive-script <file> [--archive-dir <d>]` | Timestamp-archive a teaser **script** (`beats.md` / `teaser.json` / a shot file) to `teaser/script-takes/<name>_<UTC>.<ext>` before a `--force` re-run overwrites it (Phase 6). No-op if the file is absent; the `refs/` originals are untouched, so a full pipeline re-run changes the scripts but reuses the approved portraits/plates. |
| `teaser-ffmpeg-cmd <cut_list.json> [--versioned] [--out <mp4>]` | Print the (shell-ready) ffmpeg command that stitches a cut-list into one mp4. The command body runs it via `bash`; Python never invokes ffmpeg. `--versioned` timestamps the output (`<title>_teaser_<UTC>.mp4`) + reports a `latest` pointer so a re-assemble never clobbers an earlier cut (Phase 5.8). |
