...
| `/autonovel:teaser-assemble --book <short-name> [--kind image\|video] [--audio <path>] [--audio-mode auto\|duck\|mix\|clip-only\|bed-only\|none] [--no-clip-audio] [--no-transitions] [--audio-seam-fade <s>] [--fps <n>] [--take <n>] [--force]` | standard | Stitch the rendered clips into **one teaser video** via ffmpeg. Builds an editable `cut_list.json` (ordered clips + durations + text-card notes + audio policy + transitions), runs ffmpeg, then a **viewer-panel cut critique** → `assembly-report.md`. **Audio (5.4):** native clip dialogue/music preserved; a `--audio` music bed **ducks under the dialogue** by default. **Transitions (5.7):** open fade-in / close fade-out / title fade by default (`--no-transitions` for all-cuts); `teaser-transitions` suggests where else. **Versioned (5.8):** the output mp4 is timestamped (`<title>_teaser_<UTC>.mp4`) with a `_latest` pointer so re-assembles don't clobber an earlier cut. No burned-in text (cards go in an editor); ffmpeg required. |
...
| `teaser-cut-list <teaser.json> [--clips-dir <d>] [--kind image\|video] [--audio <p>] [--audio-mode <m>] [--no-clip-audio] [--no-transitions] [--audio-seam-fade <s>] [--fps <n>] [--take <n>]` | Build an editable `cut_list.json` from teaser.json + the clips on disk (ordered clips + durations + text-card notes + audio policy + transition defaults: open/close/title fades). Plan only; reports shots with no clip. |
