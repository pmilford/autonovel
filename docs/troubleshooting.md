...
| **Timeline emoji markers (📖, 🏛) render as boxes or get dropped in the PDF / ePub** | EB Garamond and TeX Gyre Pagella don't include emoji glyphs. Switched markers to italic parentheticals: `*(in story)*`, `*(real, mentioned)*`, `*(real, context)*`. Renders as `\textit{...}` in PDF, `<em>...</em>` in ePub; works in any text font. Re-run `/autonovel:appendix --sections timeline` to regenerate the appendix with the new markers. |
...
| **TOC doesn't show chapter names even after `/autonovel:extract-chapter-titles`** | If the chapters_content.tex is rebuilt every typeset run but TOC is still numbers-only: confirm the chapter files actually have `title:` in YAML frontmatter. Run `autonovel mechanical chapter-titles books/<book>` — the report should show `✅ frontmatter` for every chapter, not `❌ missing` or `📝 # Heading`. If extract-chapter-titles ran but the report still says `❌ missing` for some chapters, the LLM wrote the title outside the YAML block — open one of the affected chapter files and confirm the `title:` line sits between `---` markers. The `--force` flag re-runs even chapters that already have a title; pass it if needed. |
