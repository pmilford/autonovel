...
| **ePub missing cover image / chapter ornaments** | The cover comes from pandoc's `--epub-cover-image=cover_titled.png` flag — confirm `books/<book>/art/cover_titled.png` exists. Per-chapter ornaments are now embedded too: `build_epub_md` emits a markdown `<img>` tag at each chapter opening when `art/ornament_chNN.png` is present, so pandoc bundles the PNG into the ePub. Re-run typeset after generating ornaments via `/autonovel:art-ornaments-all`. |
